    ...
    protected function inputData() {
        ...
        if (!MS_MODE) {
            ...
            if (preg_match('/msie|trident.+?rv\s*:/i', $_SERVER['HTTP_USER_AGENT'])) {
                ...
                exit('You are using an outdated browser version. Update to the latest version.');
            }
        }
...
//            if (!$this->template) $this->template = ADMIN_TEMPLATE . 'show';
    ...
    protected function createTableData($settings = false) {
        ...
        if (!$this->columns) new RouteException('Columns not found in the table: ' . $this->table, 2);
    ...
    protected function createOrderData($table) {
        ...
        if (!$columns)
            throw new RouteException('Fields are missing in the table ' . $table);
                ...
                if ($targetKey !== false) {
                    ...
                    if ($this->data) {
                        ...
                        $res = $this->model->get($mTable, [
                            ...
                            'where' => [
                                $this->table . '_' . $this->columns['id_row'] => $this->data[$this->columns['id_row']]
                            ]
                        ]);
